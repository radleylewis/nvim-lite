...
local function run_claude_command(command)
	local full_command = "claude " .. command
...
local function claude_with_selection()
	...
	if file then
		...
		run_claude_command("--message '" .. selected_text:gsub("'", "'\\''") .. "'")
		...
		vim.notify("Failed to create temp file for Claude", vim.log.levels.ERROR)
	end
end
...
vim.keymap.set("v", "<leader>cs", claude_with_selection, { noremap = true, silent = true, desc = "Claude Code with selection" })
